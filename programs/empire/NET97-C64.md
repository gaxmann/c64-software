...
This document summarises the reconstructed hardware and software behaviour for a 4-node C64 userport network used by “Empire C64”. The original documentation is lost. This document has been reconstructed from memory.  
...
2) ID#1 distributes the required software payload over the network:
   ...
   - SD2.75 itself
3) After distribution and “BEGIN”, clients stop TRANSM 2.75 and start:
   - SD2.75 on all nodes
...
## Next practical validation steps

- Rebuild the cable per the pinout and ID straps.
- Use a logic analyser on CNT2/SP2/PB7 during startup to confirm:
  - PB7 low while the bus is claimed
  - CNT2 burst patterns during transfers
  - SP2 data transitions aligned to CNT2 edges
- Verify ID detection by changing PB0/PB1 strap combinations and observing client behaviour.
